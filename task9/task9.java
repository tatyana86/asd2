...
class SimpleTree<T>
{
    ...
    public ArrayList<T> EvenTrees()
    {
        ...
        ArrayList<T> arrayOfNodes = new ArrayList<>();

        findNodesToBreakEdge(Root, arrayOfNodes);

        return arrayOfNodes;
    }

    private void findNodesToBreakEdge(SimpleTreeNode<T> node, ArrayList<T> arrayOfNodes) {
        for(SimpleTreeNode<T> childNode : node.Children) {
            if(countChildren(childNode) % 2 == 1) {
                arrayOfNodes.add(node.NodeValue);
                arrayOfNodes.add(childNode.NodeValue);
            }

            findNodesToBreakEdge(childNode, arrayOfNodes);
        }
    }

    private int countChildren(SimpleTreeNode<T> node) {
        return recursionGetAllNodes(node).size() - 1;
    }
}